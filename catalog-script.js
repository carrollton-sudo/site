...
document.addEventListener('DOMContentLoaded', () => {
    fetch('config.json')
        ...
        .catch(error => {
            ...
            albumList.innerHTML = "<p style='text-align:center; grid-column: 1/-1; color: #666; letter-spacing: 2px; font-size: 11px;'>NO CONFIGURATION DETECTED. USE ADMIN DASHBOARD TO INITIALIZE.</p>";
        });
});
...
function openAlbum(album) {
    ...
    let loadedImages = 0;
    const totalImages = album.photos.length;
    
    let currentSheet = null;
    let currentStrip = null;
    let framesCount = 0;
    
    album.photos.forEach((photoObj, index) => {
        // Create a new Contact Sheet Page every 12 images
        if (framesCount % 12 === 0) {
        ...
        img.src = `${album.folder}/${photoObj.file}`;
        ...
        // THIS is what makes the photos visible again
        img.onload = () => {
